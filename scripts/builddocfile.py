...
def _format_callback_param_lua(param: CallbackParam, optional_default: bool = False) -> str:
    optional = param.get("optional", True if optional_default else False)
    typ = param["type"]
    if optional:
        typ = f'({typ})?'
    result = f'{{ Type = "{typ}"'
    if param.get("name"):
        result += f', Name = "{_to_camel_case(param["name"])}"'
    result += " }"
    return result
...
def create_callback_params_lua(json_paths: list[Path], writer: TextIO, extra_config_args: list[str] = []):
    ...
    print("---@type table<string, CallbackConfig>", file=writer)
    ...
    for name, callback in callbacks.items():
        ...
        if not args and not returns:
            continue
        arg_entries = ", ".join(_format_callback_param_lua(arg) for arg in args)
        return_entries = ", ".join(_format_callback_param_lua(ret, True) for ret in returns)
        extra_config_args_str = (", " + ", ".join(extra_config_args)) if len(extra_config_args) > 0 else ""
        print(f'{DEPTH_SEP}["{name}"] = {{ Args = {{ {arg_entries} }}, Returns = {{ {return_entries} }}{extra_config_args_str} }},', file=writer)
    print("}", file=writer)
